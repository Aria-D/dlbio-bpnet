...
def split_df(df, sample_name):
    ...
    assert healthy_ids != disease_ids
...
for file in os.listdir(data_dir):
    if "GSE" in file and file.endswith(".tsv"):
        ...
        healthy_bedgraph = healthy_bedgraph[~healthy_bedgraph["chr"].str.contains(r'\.0$', regex=True)]
        #healthy_bedgraph = healthy_bedgraph.sort_values(by=["chr", "start"], key=lambda x: x if x.dtype == object else x.astype(int))
        ...
        disease_bedgraph = disease_bedgraph[~disease_bedgraph["chr"].str.contains(r'\.0$', regex=True)] 
        disease_bedgraph.to_csv(f"{outdir}/{disease}_disease.bedgraph", sep = "\t", index = None, header = None)
        #disease_bedgraph = disease_bedgraph.sort_values(by=["chr", "start"], key=lambda x: x if x.dtype == object else x.astype(int))
